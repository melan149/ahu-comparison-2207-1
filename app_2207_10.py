...
header_triggers_map = {
    ...
    get_column_safe(df, ["VDI 6022-1 certification"]): "Available configurations",
    ...
    get_column_safe(df, ["Water heater_max rows"]): "Water cooler", # Assuming this is the trigger for water cooler
    get_column_safe(df, ["Water cooler_max rows"]): "DX/DXH cooler", # Assuming this is the trigger for DX/DXH cooler
    get_column_safe(df, ["Available refrigerant types"]): "Supply Filter",
...
with st.sidebar:
    ...
    # Add capacity range columns (used only for chart)
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    for col_name in df.columns:
        ...
        # Add the column's row if it's not excluded
        if col_name not in excluded_cols_from_table:
            display_items_ordered.append({"type": "row", "col": col_name})

        # Insert charts after their respective trigger columns
        if col_name == unit_size_quantity_col:
            display_items_ordered.append({"type": "chart", "name": "unit_area_chart"})
        elif col_name == internal_height_supply_filter_col:
